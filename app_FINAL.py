...
if st.button("Calculate Emissions Savings", type="primary", use_container_width=True):
    ...
    with c1:
        st.metric("Baseline (km per person per year)", f"{vpc_c:,.0f}")
    with c2:
        st.metric("Future (km per person per year)", f"{vpc_f:,.0f}")
    with c3:
        vpc_change = vpc_f - vpc_c
        vpc_change_p = vpc_change / vpc_c * 100
        st.metric("Change per Person  (km per person per year)", f"{vpc_change:,.0f}",
    ...
    c1, c2, c3 = st.columns(3)
    with c1:
        st.metric("Baseline (kg CO2e per person per year)", f"{epc_c:,.0f}")
    with c2:
        st.metric("Future (kg CO2e per person per year)", f"{epc_f:,.0f}")
    with c3:
        epc_change = epc_f - epc_c
        epc_change_p = epc_change / epc_c * 100
        st.metric("Change per Person (kg CO2e per person per year)", f"{epc_change:,.0f}",
